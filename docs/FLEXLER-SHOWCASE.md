...
The separate earlier Release run **failed at its 25-minute build timeout**, with assembly-size optimization as its last console progress and native AOT/LLVM processes still active at cancellation. It never ran native startup, so Release remains unverified; no Release application crash was reproduced. The instrumented diagnostic gives Release compilation up to 150 minutes within one 180-minute free public job, consistent with historical full-host Simulator builds of 48–64 minutes. These limits are ceilings, not expected durations.
