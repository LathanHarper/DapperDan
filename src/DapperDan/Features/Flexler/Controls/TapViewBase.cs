...
public abstract class TapViewBase : ContentView, ITapViewBase
{
    ...
    public const string DefaultTouchSound = "Flexler/rich_touch.wav";
    public const string DefaultLongTouchSound = "Flexler/rich_long_touch.wav";
    public const string DefaultNegativeFeedbackSound = "Flexler/rich_negative_feedback.wav";
    ...
    public static readonly BindableProperty CommandParameterProperty =
        BindableProperty.Create(
            ...
            typeof(TapViewBase),
            null,
            propertyChanged: (bindable, _, _) => ((TapViewBase)bindable).RefreshCommandAvailability());

    private static readonly BindablePropertyKey IsCommandAvailablePropertyKey =
        BindableProperty.CreateReadOnly(nameof(IsCommandAvailable), typeof(bool), typeof(TapViewBase), false);

    public static readonly BindableProperty IsCommandAvailableProperty = IsCommandAvailablePropertyKey.BindableProperty;
    ...
    private bool _isCommandInvocationActive;
    ...
    public bool IsCommandAvailable => (bool)GetValue(IsCommandAvailableProperty);

    // Desktop hosts use this lifetime to preserve focus through Prism's brief
    // CanExecute=false interval while ExecuteAsync is unwinding.
    internal bool IsCommandInvocationActive
    {
        get => _isCommandInvocationActive;
        private set
        {
            if (_isCommandInvocationActive == value)
                return;
            _isCommandInvocationActive = value;
            OnPropertyChanged(nameof(IsCommandInvocationActive));
        }
    }

    protected override bool IsEnabledCore => base.IsEnabledCore && IsCommandAvailable && !IsBusy;
    ...
    protected async Task ActivateNativeTouchSequenceAsync()
    {
        ...
        catch (Exception exception)
        {
            ...
            IsCommandInvocationActive = false;
    ...
    private void RaiseTapStarting(RichButtonTapStartingEventArgs args)
    {
        try
        {
            Touching?.Invoke(this, args);
            TapStarting?.Invoke(this, args);
        }
        catch (Exception exception)
        {
            args.Cancel = true;
            _ = LogCommandExceptionAsync(exception);
        }
    }

    private static void OnCommandChanged(BindableObject bindable, object oldValue, object newValue)
    {
        var tapView = (TapViewBase)bindable;
        tapView.ObserveCommand();
        tapView.RefreshCommandAvailability();
    }
    ...
    private void ObserveCommand()
    {
        ...
        _observedCommand = _isUnloaded ? null : Command;
        if (_observedCommand is not null)
            _observedCommand.CanExecuteChanged += OnCanExecuteChanged;
    }
    ...
    private void OnCanExecuteChanged(object? sender, EventArgs args)
    {
        if (Dispatcher.IsDispatchRequired)
            Dispatcher.Dispatch(RefreshCommandAvailability);
        else
    ...
    {
        SetValue(IsCommandAvailablePropertyKey, Command?.CanExecute(CommandParameter) == true);
    ...
    {
        // Coercion preserves the owner's explicit IsEnabled binding and its
        // InputTransparent value while exposing command/busy availability to
        // native input, accessibility and the common Disabled visual state.
        RefreshIsEnabledProperty();
    }
        ...
        bool useCommandParameterSnapshot)
    {
        ...
        // Observers may change availability while reacting to a touch.
        if (!IsEnabled || InputTransparent || IsBusy)
            return;
    ...
    {
        IsCommandInvocationActive = true;
        _ownsBusy = true;
        IsBusy = true;
        IsTapping = true;
    ...
    private async Task AutoResetIsBusyAsync(int milliseconds, CancellationToken cancellationToken)
    {
        ...
        {
            if (cancellationToken.IsCancellationRequested || !_ownsBusy || !IsBusy)
                return;
    ...
    private void RequestFeedback(RichButtonFeedbackKind feedbackKind)
    {
        ...
        {
            // Device feedback is optional; unavailable audio/haptic services
            // must not prevent an otherwise valid command.
            System.Diagnostics.Debug.WriteLine($"RichButton feedback unavailable: {exception.Message}");
        }
    }
